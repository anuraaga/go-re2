...
func (re *Regexp) ReplaceAll(src, repl []byte) []byte {
	...
	srcCS := alloc.newCStringFromBytes(src)

	res, matched := re.replaceAll(&alloc, srcCS, replRE2)
	if !matched {
		return src
	}
	return res
...
func (re *Regexp) replaceAll(alloc *allocation, srcCS cString, repl []byte) ([]byte, bool) {
	replCS := alloc.newCStringFromBytes(repl)

	replCSPtr := alloc.newCStringPtr(replCS)
	srcCSPtr := alloc.newCStringPtr(srcCS)
